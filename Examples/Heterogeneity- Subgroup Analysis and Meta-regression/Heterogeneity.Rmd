...
We will discuss the output during the Working Group after this computer assignment.
...
**Note: we using AGAIN a variable that is a study characteristic, not a patient characteristic**
...
We believe that part of our data heterogeneity can be due to the country's place on the world map. That is a reasonable thing in epidemics, since the climate may effect the disease characteristics (prevalence, transmissibility, etc). Therefore, we will fit a meta-regression using the logRR as dependent variable and he absolute geographical latitude as an independent.

First, perform a random-effects meta-analysis, with DerSimonian-Laird $\tau^2$ estimate and then fit a meta-regression using the absolute geographical latitude ("ablat") as a independent variable. 
...
We can also plot the effect estimates over the range of the absolute geographical latitude, so that we may have a visual representation, using the *bubble()* command. See the help file (*??bubble.metareg*).
